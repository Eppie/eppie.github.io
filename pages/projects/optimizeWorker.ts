        ...
        if (char in layout) {
            ...
            if (prevFinger !== null) {
                ...
                if (prevPosition !== null) {
                    metrics.distanceTraveled += Math.abs(row - prevPosition[0]) + Math.abs(col - prevPosition[1]);
                }
            }
...
const tweakLayout = (layout: Layout): Layout => {
    ...
    const [key1, key2] = [keys[Math.floor(Math.random() * keys.length)], keys[Math.floor(Math.random() * keys.length)]];
...
const optimizeLayout = (
...
): { bestLayout: Layout | null, bestMetric: number, iterationsCompleted: number } => {
    ...
    for (let i = 0; i < numRestarts; i++) {
        ...
        for (let j = 0; j < iterations; j++) {
            ...
            const overallMetric = 
                ...
                weights.handBalance * Math.abs(metrics.leftHandStrokes - metrics.rightHandStrokes) + 
            ...
            if (j % 1000 === 0) {
                ...
                console.log(`Iteration ${j}: Best metric = ${bestMetric}, Temperature = ${temperature}, Acceptance rate = ${acceptanceRate}`);
            }
        }
    ...
    return { bestLayout: bestLayoutOverall, bestMetric: bestMetricOverall, iterationsCompleted };
};

// Listen for messages from the main thread
onmessage = (e) => {
    const { layout, text, initialTemperature, coolingRate, iterations, numRestarts, weights } = e.data;
    const result = optimizeLayout(layout, text, initialTemperature, coolingRate, iterations, numRestarts, weights);
    postMessage({
        type: 'result',
