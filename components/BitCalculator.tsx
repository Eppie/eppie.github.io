...
export const operate = (left: number, right: number, operator: string): number => {
    switch (operator) {
        case '&':
            return left & right;
        case '|':
...
export const evaluatePostfix = (postfix: string[]): Array<{ operator: string, value: number, binary: string }> => {
    const stack: number[] = [];
    ...
    for (const token of postfix) {
        if (/^\d+$/.test(token)) {
            ...
            if (token === '~') {
                const a = stack.pop();
                if (a === undefined) {
                    throw new Error("Invalid expression: missing operand for '~' operator");
                }
                const result = operate(a, 0, token);
                results.push({operator: token, value: result, binary: toBinaryString(result)});
                stack.push(result);
            } else {
                const b = stack.pop();
                const a = stack.pop();
                if (a === undefined || b === undefined) {
                    throw new Error(`Invalid expression: missing operands for '${token}' operator`);
                }
                ...
                results.push({operator: "=", value: result, binary: toBinaryString(result)});
                stack.push(result);
            }
        }
    }
    if (stack.length === 1) {
        ...
        results.push({operator: '=', value: finalValue, binary: toBinaryString(finalValue)});
    }
    return results;
};
...
const BitCalculator: React.FC = () => {
    ...
    const [results, setResults] = useState<Array<{ operator: string, value: number, binary: string }>>([]);
                ...
                <table style={{width: '100%', borderCollapse: 'collapse'}}>
                    ...
                    {results.map((result, index) => (
                        <tr key={index} style={{borderBottom: '1px solid #ccc'}}>
                            <td style={{width: '20px', padding: '5px'}}>{result.operator}</td>
                            <td style={{
                                width: '50px',
                                padding: '5px',
                                textAlign: 'right'
                            }}>{!isNaN(result.value) ? result.value : ''}</td>
