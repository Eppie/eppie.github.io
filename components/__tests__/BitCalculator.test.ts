...
describe('Bitwise Calculator Functions', () => {
  ...
  describe('tokenize', () => {
    ...
    it('should handle complex expressions', () => {
      expect(tokenize('5 << 2 | 3 ^ ~1')).toEqual([
        '5',
        '<<',
        '2',
        '|',
  ...
  describe('infixToPostfix', () => {
    ...
    it('should handle parentheses correctly', () => {
      expect(infixToPostfix(['(', '5', '&', '3', ')', '|', '2'])).toEqual([
        '5',
        '3',
        '&',
        '2',
    ...
    it('should handle operator precedence correctly', () => {
      expect(infixToPostfix(['5', '<<', '2', '|', '3', '^', '~', '1'])).toEqual(
        ['5', '2', '<<', '3', '1', '~', '^', '|']
      );
    });
  });
  ...
  describe('evaluatePostfix', () => {
    ...
    it('should throw an error for invalid expressions', () => {
      expect(() => evaluatePostfix(['5', '&'])).toThrowError(
        "Invalid expression: missing operands for '&' operator"
      );
      expect(() => evaluatePostfix(['~'])).toThrowError(
